...
from ..assistant.client import build_default_client
...
class DashboardService:
    ...
    def run_magic_rewrite(self, target_url: str) -> dict[str, Any]:
        """Dynamically generate a new description format using DeepSeek LLM."""
        state = self._load_dashboard_state()
        base_snapshot = self._load_snapshot()
        client = build_default_client()
        
        comparison = state.gig_comparison or {}
        blueprint = comparison.get("implementation_blueprint") or {}
        options = blueprint.get("description_options", [])
        
        prompt = (
            f"You are an expert copywriter. Rewrite this Fiverr gig description to be persuasive, "
            f"clear, and exactly 1000-1200 characters long. Focus on buyer intent and clear scope.\n"
            f"Original Title: {base_snapshot.title}\n"
            f"Original Description: {base_snapshot.description}\n"
            f"Output the new description in plain text only, no conversational filler."
        )
        try:
            result = client.complete(prompt, temperature=0.7)
            new_text = result.strip() if hasattr(result, "strip") else str(result)
        except Exception as e:
            new_text = f"Magic Rewrite failed: {str(e)}"
            
        new_option = {
            "label": "✨ DeepSeek Magic Rewrite",
            "summary": "Dynamically generated by your local AI assistant.",
            "text": new_text,
            "paired_title": base_snapshot.title,
        }
        
        # Filter out previous magic rewrites to avoid duplicates, then prepend the new one
        filtered_options = [opt for opt in options if opt.get("label") != "✨ DeepSeek Magic Rewrite"]
        filtered_options.insert(0, new_option)
        blueprint["description_options"] = filtered_options
        comparison["implementation_blueprint"] = blueprint
