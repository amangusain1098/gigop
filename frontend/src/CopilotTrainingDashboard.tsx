...
type CsrfRefresher = () => Promise<string>
...
interface CorpusDoc {
  id: string; source: string; source_type: string
  ingested_at: string; token_count: number; text_preview: string
}
...
function kpiCard(color?: string): React.CSSProperties {
  return {
    ...
    borderTop: `3px solid ${color ?? '#e2e8f0'}`,
  }
}
...
const labelStyle: React.CSSProperties = {
  display: 'flex', flexDirection: 'column', fontSize: 13, fontWeight: 600, color: '#374151',
}

type Panel = 'overview' | 'predict' | 'tests' | 'corpus' | 'schedule' | 'ingest'

export default function CopilotTrainingDashboard({ csrfToken, onCsrfRefresh }: Props) {
  ...
  /** POST / PUT with CSRF token + automatic retry on token expiry. */
  const postJson = useCallback(async <T,>(
    url: string, body: unknown, method = 'POST',
  ): Promise<T> => {
    const send = (tok: string) => fetchJson<T>(url, {
      method,
    ...
    try {
      ...
      if (/csrf/i.test(err instanceof Error ? err.message : String(err))) {
        return send(await onCsrfRefresh())
      }
      throw err
    }
  }, [csrfToken, onCsrfRefresh])
  ...
  async function runTraining() {
    setBusy('train')
    try {
      const r = await postJson<{ steps: unknown[] }>('/api/copilot/training-dashboard/train', {})
  ...
  async function runTests() {
    setBusy('tests'); setTestOutput('')
    try {
      const r = await postJson<TestResults>('/api/copilot/training-dashboard/run-tests', {})
  ...
  async function submitIngest() {
    ...
    setBusy('ingest')
    try {
      const r = await postJson<{ ingested: boolean; tokens_learned?: number; new_words?: number }>(
        '/api/copilot/training-dashboard/ingest',
        { text: ingestText, source: ingestSource || 'manual-ui', source_type: 'manual' },
      )
      if (r.ingested) {
  ...
  async function saveSchedule() {
    setBusy('schedule')
    try {
      await postJson('/api/copilot/training-dashboard/schedule',
        { interval: scheduleInterval, enabled: scheduleEnabled }, 'PUT')
  ...
  if (loading && !stats) return (
    <div style={{ padding: 40, textAlign: 'center', color: '#94a3b8' }}>
      Loading Copilot Training Dashboard…
    </div>
  )
  if (loadError) return (
    ...
    <div style={{ fontFamily: 'system-ui, sans-serif', maxWidth: 1100, margin: '0 auto', padding: '24px 16px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 12, marginBottom: 24 }}>
        {([
          ...
          { label: 'Documents',      value: (s.totals.total_docs ?? 0).toLocaleString(),      icon: '📄', color: '#8b5cf6' },
          { label: 'Tokens Learned', value: (s.totals.total_tokens ?? 0).toLocaleString(),   icon: '🔤', color: '#06b6d4' },
          { label: 'New Words',      value: (s.totals.total_new_words ?? 0).toLocaleString(),icon: '✨', color: '#f59e0b' },
          { label: 'Bigrams',        value: s.model.bigram_count.toLocaleString(),            icon: '🔗', color: '#64748b' },
          { label: 'Cron Cycles',    value: (sc.run_count ?? 0).toString(),                   icon: '⏰', color: '#64748b' },
          {
            label: 'Tests',
      ...
      {activePanel === 'overview' && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
          ...
          <div style={panelStyle}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              ...
              {s.model.top_words.length === 0 && (
                <p style={{ color: '#94a3b8', fontSize: 13 }}>No vocabulary yet — run a training cycle.</p>
              )}
            </div>
          </div>
                ...
                <div key={ev.event_id} style={{ display: 'flex', gap: 10, alignItems: 'flex-start', fontSize: 12, marginBottom: 8 }}>
                  ...
                  <div>
                    ...
                    <div style={{ color: '#64748b' }}>
                      {ev.tokens_learned != null ? `${ev.tokens_learned} tokens · ${ev.new_words ?? 0} new · ` : ''}
                      {ts(ev.timestamp)}
                    </div>
                  </div>
                </div>
            ...
            <table style={{ width: '100%', fontSize: 13, borderCollapse: 'collapse' }}>
              <tbody>
                {([
                  ...
                  ['Cycles Run', (sc.run_count ?? 0).toString()],
          ...
          <div style={panelStyle}>
            ...
            {tr.status === 'never_run'
              ? <div>
                  <p style={{ color: '#94a3b8', fontSize: 13, marginBottom: 12 }}>Tests not run yet.</p>
                  <button onClick={() => { setActivePanel('tests'); void runTests() }} style={btnStyle('#3b82f6')}>Run Tests Now</button>
                </div>
              : <div>
                  <div style={{ display: 'flex', gap: 16, marginBottom: 12 }}>
                    {([
                      ['Total',  tr.total,  '#64748b'],
                      ['Passed', tr.passed, '#22c55e'],
                      ['Failed', tr.failed, '#ef4444'],
                      ['Errors', tr.errors, '#f59e0b'],
        ...
        <div style={panelStyle}>
          ...
          {completions.length > 0 && (
            <div style={{ marginTop: 16 }}>
              ...
              {completions.map((c, i) => (
                <div key={i} onClick={() => setPredictQuery(c.full_suggestion)} style={{
                  display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px',
                  marginBottom: 6, borderRadius: 8, background: '#f8fafc',
          ...
          {s.model.vocab_size === 0 && (
            <div style={{ background: '#fef9c3', border: '1px solid #fde047', borderRadius: 8, padding: 14, marginTop: 20, fontSize: 13 }}>
              Vocabulary is empty. Run a Training Cycle first.
            </div>
          )}
        </div>
      )}
      ...
      {activePanel === 'corpus' && (
        <div style={panelStyle}>
          ...
          {s.corpus_docs.length === 0
            ? <p style={{ color: '#94a3b8', fontSize: 13 }}>No documents yet. Use the Ingest tab.</p>
      ...
      {activePanel === 'schedule' && (
        <div style={panelStyle}>
          ...
          <p style={{ color: '#64748b', fontSize: 13, marginBottom: 20 }}>
            The copilot automatically scans conversations and ingests new Q&amp;A pairs on this schedule.
          </p>
            ...
            <table style={{ fontSize: 13, borderCollapse: 'collapse', width: '100%' }}>
              <tbody>
                {([
                  ...
                  ['Last Run',     ts(sc.last_run)],
                  ['Next Run',     ts(sc.next_run)],
                  ['Total Cycles', (sc.run_count ?? 0).toString()],
                ] as [string, string][]).map(([k, v]) => (
      ...
      {activePanel === 'ingest' && (
        <div style={panelStyle}>
          ...
          <p style={{ color: '#64748b', fontSize: 13, marginBottom: 20 }}>
            Paste any text — gig descriptions, FAQs, conversations — to teach the copilot new vocabulary.
          </p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14, maxWidth: 700 }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={() => { setIngestText(''); setIngestSource('') }} style={btnStyle('#64748b')}>
                Clear
              </button>
            </div>
            <div style={{ background: '#f0fdf4', border: '1px solid #86efac', borderRadius: 8, padding: 12, fontSize: 12, color: '#166534' }}>
              <strong>Tip:</strong> After ingesting, switch to Word Prediction to verify new completions work.
            </div>
          </div>
        </div>
      )}
