...
import { EmptyState, type CompetitorRecommendation, currency, displayDiffValue, shortDate } from './helpers'
...
interface CompetitorPageProps {
  ...
  timeline: Array<Omit<TimelinePoint, 'label'>>
  timelineChart: TimelinePoint[]
  ...
  return (
    <>
      <section className="charts">
        ...
        <article className="card">
          <div className="card-head"><h2>Competitor table</h2>
          ...
          <div className="table">
            {competitors.length ? competitors.map((item) => (
              ...
              <EmptyState icon="📭" title="No competitors found yet" hint={comparisonMessage || 'Run a gig analysis to populate this table.'} />
            )}
          </div>
        </article>
      </section>
        ...
        <article className="card">
          ...
          <div className="table">
            {pageOneTopTen.length ? pageOneTopTen.map((item) => (
              ...
              <EmptyState icon="🔎" title="No page-one gigs yet" hint={comparisonMessage || 'Try a more specific search phrase.'} />
            )}
          </div>
        </article>
        ...
        <article className="card">
          ...
          <div className="feed-list">
            {oneByOne.length ? oneByOne.map((item) => (
              ...
              <EmptyState icon="🎯" title="No one-by-one recommendations yet" hint="Run a compare to generate actions against page-one gigs." />
            )}
          </div>
        </article>
      </section>
      ...
      <section className="content-grid">
        <article className="card">
          ...
          <div className="chart-shell">
            {timelineChart.length ? (
              ...
              <EmptyState icon="🕒" title="No timeline yet" hint="Comparison runs will appear here once you compare the same gig more than once." />
            )}
          </div>
          <div className="table">
        ...
        <article className="card">
          ...
          <div className="meta-grid">
            <div className="meta-item"><span>Earlier run</span><strong>{comparisonDiff.left?.created_at ? shortDate(String(comparisonDiff.left.created_at)) : '--'}</strong></div>
            <div className="meta-item"><span>Later run</span><strong>{comparisonDiff.right?.created_at ? shortDate(String(comparisonDiff.right.created_at)) : '--'}</strong></div>
          </div>
          <div className="table">
            {comparisonDiff.available && comparisonDiff.changes.length ? comparisonDiff.changes.map((change) => (
              ...
              <EmptyState icon="🧭" title="No diff ready yet" hint="Run another comparison to see how the market recommendations changed over time." />
            )}
          </div>
        </article>
      </section>
